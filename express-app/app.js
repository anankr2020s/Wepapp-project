...
const url = 'mongodb://localhost:27017/bookmarket'
const config = {
    autoIndex: true,
...
const getSomeProduct = (pid) =>{
    return new Promise ((resolve, reject) =>{
        Product.find({"id":pid}, (err,data) =>{
            if(err){
                reject(new Error('Cannot get products'));
            }else{
                if(data){
...
expressApp.get('/products/get/:id',(req,res) =>{
    console.log('get');
    getSomeProduct(req.params.id)
        .then(result =>{
            console.log(result);
            res.status(200).json(result);
        })
